...
myApp.controller('MainCtrl', function ($scope, $http) {
    ...
    */
    ...
    $scope.getDataBook = function () {
        ...
        $http.post('/getMagazineDetails').success(function (gotBooks) {
            var p = [];
        ...
        $http.post('/getMagazineBillNo').success(function (billArray) {
            x = parseInt(billArray[0].billno);
            x = x + 1;
    ...
    $scope.add = function (bookcode) {
        document.getElementById('issn').value = $scope.books[bookcode - 1].issn;
        document.getElementById('title').value = $scope.books[bookcode - 1].title;
        document.getElementById('editor').value = $scope.books[bookcode - 1].editor;
        document.getElementById('price').value = $scope.books[bookcode - 1].price;
        document.getElementById('discount').value = $scope.books[bookcode - 1].discount;
    }
    ...
    $scope.order = function () {
        ...
        books = JSON.parse(json);
        alert(books);
        $http.post('/addMagazineOrder', books).success(function (books) {
            //alert(books);
            alert("Order Placed");
        });
    }
